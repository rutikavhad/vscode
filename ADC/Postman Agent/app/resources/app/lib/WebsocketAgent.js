const fs = require('fs'),
  ...
  setupRequestResolutionService = require('../services/WSRequestResolutionService').initialize,
  CryptoService = require('./common/services/CryptoService'),
  VaultScriptService = require('./common/services/VaultScriptService'),
  { ServerMethods } = require('./common/services/UnifiedRuntimeService'),
...
function start (R, done) {
  ...
  io.on('connection', (socket) => {
    ...
    setupRequestResolutionService(socket);
    ...
    socket.on('agent-updates', (fn, args, cb) => {
      ...
      if (fn === 'getStatus') {
        ...
        agentUpdateService.restartAndUpdate(...args);
      }
    });
