...
PostmanFs.prototype.stat = function (path, callback) {
  ...
  if (this._isFileRemote(path) && pm.wsCloudFileService) {
    return pm.wsCloudFileService.fetchFile(path, (err, filePath) => {
      if (err) {
        return callback(err);
      }

      this.resolvePath(filePath, (err, resolvedPath) => {
        if (err) {
          return callback(err);
        }
...
PostmanFs.prototype.createReadStream = function (path, options) {
    try {
      ...
      if (this._isFileRemote(path) && remoteFilePathMap.has(path)) {
