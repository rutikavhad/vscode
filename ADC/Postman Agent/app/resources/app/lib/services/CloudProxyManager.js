...
// constants
const { DEFAULT_FONTS } = require('../constants/WindowConstants');
...
const AUTH_WINDOW_OPTIONS = {
  ...
  webPreferences: {
    ...
    preload: path.resolve(app.getAppPath(), 'preload_desktop.js'),
    defaultFontFamily: DEFAULT_FONTS[process.platform]
  }
};
...
class CloudProxyManager {
  ...
  getProxyAuthHTML (pmBuildEnv = process.env.PM_BUILD_ENV) {
    let htmlPath = path.resolve(app.getAppPath(), 'html');
