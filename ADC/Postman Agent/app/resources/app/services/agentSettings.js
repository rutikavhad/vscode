...
module.exports = {
  ...
  getAll (cb) {
    Storage.get(DB_KEY, (error, data) => {
      if (error) { pm.logger.error('Postman Agent~agentSettings~getAll - Failed to get data from storage', error); }
  ...
  get (key, cb) {
    Storage.get(DB_KEY, (error, data) => {
      if (error) { pm.logger.error('Postman Agent~agentSettings~get - Failed to get data from storage', key, error); }
    ...
    this.getAll((err, data) => {
      if (!err) {
        ...
        Storage.set(DB_KEY, data, (error) => {
          if (error) { pm.logger.error('Postman Agent~agentSettings~set - Failed to store data on storage', error); }
          return cb && cb(error, data);
        });
      }
      else {
        if (err) { pm.logger.error('Postman Agent~agentSettings~set - Failed to get data from storage', err); }
