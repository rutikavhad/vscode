...
/**
 * agentUpdaterAdapter is a bridge between the updater and the electron app.
 * This abstracts the internal implementation of app away from the updater module.
 */
let agentUpdaterAdapter = {
    ...
    getAppName () {
      const appName = pm.config.get('AppName');
      return appName;
    },

    getLogger () {
      return pm.logger || console;
    },
    ...
    /**
     * This will return the path of the Postman Agent directory
     *
     * some-folder
     *   postman agent        <---------- getAppInstallationPath()
     *     app
     *     |  resources
     *     |    app      <----------- electron.app.getAppPath()
     *     |      main.js
     *     |      package.json
     *     postman agent-xxxxxxxxxx.tar.gz
     *     postman agent-xxxxxxxxxx-old
     */
    ...
    getLastKnownVersion (cb) {
      agentSettings.get('lastKnownVersion', cb);
    },
    ...
    setLastKnownVersion (version) {
      agentSettings.set('lastKnownVersion', version);
    },
    ...
    setDownloadedVersion (version) {
      agentSettings.set('downloadedVersion', version);
    },
...
module.exports = agentUpdaterAdapter;
