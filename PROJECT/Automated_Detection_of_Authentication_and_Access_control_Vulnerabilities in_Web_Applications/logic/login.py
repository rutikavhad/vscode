...
from logic.word_list.payload_list import sql_payloads,no_sql_payloads
from logic.no_login_attacks.rate_limit import test_rate_limit
from logic.no_login_attacks.sql_injection import run_sqli_scan
from logic.no_login_attacks.brute_force import run_brute_scan
...
def scan(url):
    try:
        ...
        print(sqlresults)
        print("===================================================================")
        print("NO SQL INNJECTIONS")
        print("===================================================================")
        sqlresults = run_sqli_scan(
            form, 
            payloads=no_sql_payloads,
            threads=10
        )
        print(sqlresults)

        bruteresult=run_brute_scan(
            form,
        )
        print(bruteresult)
        print("\n[DONE]")
...
if __name__ == "__main__":
    ...
    # url = sys.argv[1] if len(sys.argv) > 1 else "http://127.0.0.1:3000/juice-shop/
    # url = sys.argv[1] if len(sys.argv) > 1 else "http://127.0.0.1:3000/juice-shop#/login"
